...
class Weak2StrongExplanation:
    ...
    def get_forward_info(self, inputs_dataset, class_label, debug=True):
        ...
        for _, i in enumerate(inputs_dataset):
            ...
            if debug and _ > 499:
                break
            list_hs, tl_pair = step_forward(self.model, self.tokenizer, i) # 返回每一层的隐藏状态和下一个可能的输出
            # print("----list_hs:---")
            # print(list_hs)
            # print("----tl_pair:---")
            # print(tl_pair)
            last_hs = [hs[:, -1, :] for hs in list_hs]
            self.forward_info[_ + offset] = {"hidden_states": last_hs, "top-value_pair": tl_pair, "label": class_label} # 记录所有数据集的每一层隐藏状态、下一个可能的输出
